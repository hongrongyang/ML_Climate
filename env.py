...
class Env():
    def __init__(self):
        #setting parameters
        self.obs, self.net, self.origin_p31, self.origin_p13, self.origin_p19, p31, p13, p19 = obs()
        self.scale = 1000 #10*1000
        self.t = 1
        self.vc = 0.5
        self.charging_power = 60
        ...
        # self.ratio2 = [0.85, 0.87, 0.9, 0.92, 0.94, 0.98, 1, 1.1, 1.05, 1, 0.97, 0.95, 0.92]
    ...
    def step(self, action):
        ...
        # z = ev_true(self.demand[self.t-1], self.t, 1)
        # for i in range(2, 14):
        #     z = np.vstack((z, ev_true(self.demand[self.t-1], self.t, i)))
        ...
        demand_number = self.demand1[self.t-1] + self.demand2[self.t - 1] + self.demand3[self.t - 1] + self.demand4[self.t - 1] \
                        + self.demand5[self.t - 1] + self.demand6[self.t - 1] + self.demand7[self.t - 1] + self.demand8[self.t - 1]\
                        + self.demand9[self.t - 1] + self.demand10[self.t - 1] + self.demand11[self.t - 1] + self.demand12[self.t - 1] + self.demand13[self.t - 1]
        ...
        if self.t == 1:
            ...
            # print((Decimal(math.factorial(k-1))*Decimal((k-lamda_t*average_S))))
                b = (lamda_t*average_S)**i/math.factorial(i) + (lamda_t*average_S)**self.cn1/(math.factorial(self.cn1-1)*(self.cn1-lamda_t*average_S))
                a = a + b
            t_waiting1 = float((lamda_t**self.cn1)*(average_S**(self.cn1-1))*average_S2/(2*math.factorial(self.cn1-1)*((self.cn1-lamda_t*average_S)**2)*a))
            if t_waiting1 < 0:
            ...
            # print((Decimal(math.factorial(k-1))*Decimal((k-lamda_t*average_S))))
                b = (lamda_t*average_S)**i/math.factorial(i) + (lamda_t*average_S)**self.cn2/(math.factorial(self.cn2-1)*(self.cn2-lamda_t*average_S))
                a = a + b
            t_waiting2 = float((lamda_t**self.cn2)*(average_S**(self.cn2-1))*average_S2/(2*math.factorial(self.cn2-1)*((self.cn2-lamda_t*average_S)**2)*a))
            if t_waiting2 < 0:
            ...
            # print((Decimal(math.factorial(k-1))*Decimal((k-lamda_t*average_S))))
                b = (lamda_t*average_S)**i/math.factorial(i) + (lamda_t*average_S)**self.cn3/(math.factorial(self.cn3-1)*(self.cn3-lamda_t*average_S))
                a = a + b
            t_waiting3 = float((lamda_t**self.cn3)*(average_S**(self.cn3-1))*average_S2/(2*math.factorial(self.cn3-1)*((self.cn3-lamda_t*average_S)**2)*a))
            if t_waiting3 < 0:
        ...
        #elastic demand
        h = 0
        # for k in range(self.demand[self.t - 1] * 13):
        #     if min(t_total1[k], t_total2[k], t_total3[k]) < z[k, 0]:
        #         c_total1[k] = t_total1[k]
        #         c_total2[k] = t_total2[k]
        #         c_total3[k] = t_total3[k]
        ...
        # print(self.n_arrival1, self.n_arrival2, self.n_arrival3)
        # print(len(self.cs1),len(self.cs2),len(self.cs3))
        ...
        if self.t > 1:
            for i in range(0, 12):
                self.net.load.iloc[i, 6] = self.net.load.iloc[i, 6] / self.ratio[self.t - 2] * self.ratio[self.t - 1]
            for i in range(13, 18):
                self.net.load.iloc[i, 6] = self.net.load.iloc[i, 6] / self.ratio[self.t - 2] * self.ratio[self.t - 1]
            for i in range(19, 30):
                self.net.load.iloc[i, 6] = self.net.load.iloc[i, 6] / self.ratio[self.t - 2] * self.ratio[self.t - 1]
            self.net.load.iloc[31, 6] = self.net.load.iloc[31, 6] / self.ratio[self.t - 2] * self.ratio[self.t - 1]
        ...
        self.net.load.iloc[30, 6] = self.origin_p31 * self.ratio[self.t-1] + p_cs1
        ...
        self.net.load.iloc[12, 6] = self.origin_p13 * self.ratio[self.t-1] + p_cs2
        ...
        self.net.load.iloc[18, 6] = self.origin_p19 * self.ratio[self.t-1] + p_cs3
        ...
        p31 = self.net.load.iloc[30, 6] * self.scale / 1000
        p13 = self.net.load.iloc[12, 6] * self.scale / 1000
        p19 = self.net.load.iloc[18, 6] * self.scale / 1000
        v31 = self.net.res_bus.loc[31, "vm_pu"]
        v13 = self.net.res_bus.loc[13, "vm_pu"]
        v19 = self.net.res_bus.loc[19, "vm_pu"]
        self.p31.append(p31)
        self.p13.append(p13)
        self.p19.append(p19)
        # self.v31.append(v31)
        ...
        #     self.congestion_error += load_percentage9 - 0.9
        # if load_percentage17 >= 0.9:
        #     self.congestion_error += load_percentage17 - 0.9
        # if load_percentage27 >= 0.9:
        #     self.congestion_error += load_percentage27 - 0.9

        # if v13 < 0.95:
        #     self.voltage_error += 0.95 - v13
        # elif v19 < 0.95:
        #     self.voltage_error += 0.95 - v19
        # elif v31 < 0.95:
        #     self.voltage_error += 0.95 - v31
        # elif v13 > 1.05:
        #     self.voltage_error += v13 - 1.05
        # elif v19 > 1.05:
        #     self.voltage_error += v19 - 1.05
        # elif v31 > 1.05:
        #     self.voltage_error += v31 - 1.05
        # else:
        #     self.voltage_error += 0
        ...
        for i in range(self.n1):
            if self.cs1[:, 5][i] < self.charging_power/4:
                cs1_revenue += self.cs1[:, 5][i] * self.price1[int(self.cs1[:, 6][i])]
                cs1_profits += self.cs1[:, 5][i] * (self.price1[int(self.cs1[:, 6][i])] - self.electricity_purchase_CS)
            else:
                cs1_revenue += 15 * self.price1[int(self.cs1[:, 6][i])]
                cs1_profits += 15 * (self.price1[int(self.cs1[:, 6][i])] - self.electricity_purchase_CS)
        self.cs1_profits.append(cs1_profits)

        for i in range(self.n2):
            if self.cs2[:, 5][i] < self.charging_power/4:
                cs2_revenue += self.cs2[:, 5][i] * self.price2[int(self.cs2[:, 6][i])]
                cs2_profits += self.cs2[:, 5][i] * (self.price2[int(self.cs2[:, 6][i])] - self.electricity_purchase_CS)
            else:
                cs2_revenue += 15 * self.price2[int(self.cs2[:, 6][i])]
                cs2_profits += 15 * (self.price2[int(self.cs2[:, 6][i])] - self.electricity_purchase_CS)
        self.cs2_profits.append(cs2_profits)

        for i in range(self.n3):
            if self.cs3[:, 5][i] < self.charging_power/4:
                cs3_revenue += self.cs3[:, 5][i] * self.price3[int(self.cs3[:, 6][i])]
                cs3_profits += self.cs3[:, 5][i] * (self.price3[int(self.cs3[:, 6][i])] - self.electricity_purchase_CS)
            else:
                cs3_revenue += 15 * self.price3[int(self.cs3[:, 6][i])]
                cs3_profits += 15 * (self.price3[int(self.cs3[:, 6][i])] - self.electricity_purchase_CS)
        ...
        self.obs.append(social_welfare / 10)
        # self.obs.append(p31)
        # self.obs.append(p13)
        # self.obs.append(p19)
        ...
        #     cost_v19 += 0
        # 
        # cost2 = cost_v31 + cost_v13 + cost_v19
        # 
        #3.profits reward
        reward3 = social_welfare

        self.reward = reward3 / 60
        # self.cost = cost1*1000 + cost2*5000
        self.cost = cost1 * 100
        # print(self.reward, self.cost)
        # self.reward = self.reward - self.cost
