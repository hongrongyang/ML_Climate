...
class ConstrainedCEM:
    def __init__(self,
        ...
        self.dO, self.dU = 8, 3
        self.ac_ub, self.ac_lb = np.array([1.4, 1.5, 1.4]), np.array([1.1, 1.3, 1.2])
        self.gamma = gamma
        ...
        if not self.learn_kappa:
            ...
            self.kappa_optim = Adam([self.kappa], lr=0.01)
        ...
        self.init_sol = (self.ac_lb + self.ac_ub) / 2
    ...
    def select_action(self, obs, eval_t=False):
        ...
        if self.ac_buf.shape[0] > 0:
            ...
            return action
        # print('self.prev_sol', self.prev_sol)
        # print('self.init_var', self.init_var)
        soln = self.obtain_solution(obs, self.prev_sol, self.init_var)
        # print(soln)
        self.prev_sol = np.concatenate([np.copy(soln)[self.per * self.dU:], self.init_sol])
        # print('prev_sol1', self.prev_sol, np.shape(self.prev_sol))
        self.ac_buf = soln[:self.per * self.dU].reshape(-1, self.dU)
        # print('ac_buf', self.ac_buf)

        return self.select_action(obs)

    def obtain_solution(self, obs, init_mean, init_var):
        mean, var, t = init_mean, init_var, 0
        X = stats.truncnorm(-3, 3, loc=np.zeros_like(mean), scale=np.ones_like(var))
        while (t < self.max_iters) and np.max(var) > self.epsilon:
            ...
            # samples = noise * 0 + mean
            samples = samples.astype(np.float32)
            #samples shape(400,36)
            ...
            # print('reward', rewards[elite_ids])
    ...
    def rollout(self, obs, ac_seqs):
        ...
        # Before, ac seqs has dimension (400, 36) which are pop size and sol dim coming from CEM
        ac_seqs = ac_seqs.view(-1, self.plan_hor, self.dU)
        #  After, ac seqs has dimension (400, 12, 3)
    ...
    def optimize_kappa(self, episode_cost, permissible_cost=None):
        ...
        np.save('./data/kappa', self.kappa.detach().numpy())
