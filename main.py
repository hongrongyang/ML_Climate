...
# import gym
# from gym.wrappers import Monitorr
...
def readParser():
    ...
    parser.add_argument('--model_retain_epochs', type=int, default=12, metavar='A',
                    help='retain epochs')
    parser.add_argument('--model_train_freq', type=int, default=12, metavar='A',
                    help='frequency of training')
    parser.add_argument('--epoch_length', type=int, default=12, metavar='A',
                    help='steps per epoch')
    parser.add_argument('--num_epoch', type=int, default=1, metavar='A',
    ...
    parser.add_argument('--hidden_size', type=int, default=256, metavar='A',
...
def train(args, env_sampler, env_model, cem_agent, env_pool):
    ...
    for epoch_step in tqdm(range(args.num_epoch)):
        ...
        for i in range(args.epoch_length):
            ...
            # epoch_rewards[-1] += args.gamma ** i * reward[0]
...
def main():
    ...
    env_model = ProbEnsemble(state_size, action_size, network_size=5, cuda=args.cuda,
                             cost=args.learn_cost, binary_cost=args.binary_cost, hidden_size=args.hidden_size, name='model')
    if args.cuda:
        env_model.to('cuda')
